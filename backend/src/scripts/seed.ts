...
const getProfilePic = (gender: string, index: number) => {
...
const seed = async () => {
  ...
  const existingCount = await User.countDocuments({ username: { $in: userProfiles.map(u => u.username) } });
  if (existingCount > 0) {
    ...
    if (process.argv.includes('--clean')) {
      ...
      const seedUsers = await User.find({ username: { $in: userProfiles.map(u => u.username) } }).select('_id');
  ...
  for (const user of users) {
    const othersToFollow = shuffle(users.filter((u: any) => u._id.toString() !== (user as any)._id.toString()))
      .slice(0, rand(5, 15));

    const followIds = othersToFollow.map(u => u._id);
    await User.findByIdAndUpdate(user._id, { $set: { following: followIds } });

    // Add this user to each followed user's followers
    for (const followedUser of othersToFollow) {
      await User.findByIdAndUpdate(followedUser._id, { $addToSet: { followers: user._id } });
    }
  }
  ...
  for (const user of users) {
    ...
    await User.findByIdAndUpdate(user._id, { $set: { bookmarks: bookmarked.map(p => p._id) } });
  }
